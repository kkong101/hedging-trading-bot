...
func NewTelegram(controller *order.Controller, settings model.Settings, options ...Option) (_service.Telegram, error) {
...
func (t telegram) BuyHandle(m *tb.Message) {
	...
	order, err := t.orderController.CreateOrderMarketQuote(futures.SideTypeBuy, pair, amount)
	if err != nil {
		return
	}
